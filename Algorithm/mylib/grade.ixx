...
export namespace mylib {
	...
	struct grade {
		...
		grade(long long _Numerator, long long _Denominator = 1) noexcept {
		...
		grade(const grade&) noexcept = default;
		grade() noexcept = default;

		grade operator -() const noexcept {
			return grade{ -numerator, denominator };
		}

		grade operator +(const grade& _R) const noexcept {
	...
	template<typename _ElementType>
		requires std::is_integral_v<_ElementType>
	bool operator < (const grade<_ElementType>& _L, const grade<_ElementType>& _R) noexcept {
