...
#   else
    ...
    namespace detail {
        ...
        constexpr auto test_is_convertible_v(int) noexcept ->
            decltype(static_cast<To(*)(To)>(nullptr)(declval<From>()), true_type{})
        {}
...
#   else
    ...
    namespace detail {
        ...
        constexpr auto test_is_nothrow_convertible_v(int) noexcept ->
            bool_constant<
                noexcept(
                    static_cast<To(*)(To) noexcept>(nullptr)(declval<From>())
                )
            >
        {}
...
#   ifdef MYLIB_UNITTEST
    ...
    namespace unittest {
        ...
        constexpr bool assert_test_type_traits() noexcept {
            ...
            static_assert( is_integral_v<const volatile short>);
            ...
            static_assert(!is_integral_v<long double>);
            ...
            // test is_array_v
            static_assert( is_array_v<char[3]>);
            static_assert( is_array_v<long long[]>);
            static_assert( is_array_v<const int[3]>);
            static_assert( is_array_v<volatile double[3]>);
            static_assert( is_array_v<const volatile long long[3]>);

            static_assert(!is_array_v<int>);
            static_assert(!is_array_v<void>);
            static_assert(!is_array_v<char*>);
            static_assert(!is_array_v<nullptr_t>);
            static_assert(!is_array_v<long long>);

            // test is_pointer_v
            static_assert( is_pointer_v<int*>);
            static_assert( is_pointer_v<const int*>);
            static_assert( is_pointer_v<long long*>);

            static_assert(!is_pointer_v<int>);
            static_assert(!is_pointer_v<void>);
            static_assert(!is_pointer_v<long long>);
            static_assert(!is_pointer_v<nullptr_t>);
            ...
            static_assert( is_enum_v<Enum1>);
            static_assert( is_enum_v<Enum2>);
            static_assert( is_enum_v<EnumClass2>);
            static_assert( is_enum_v<EnumClass1>);
            static_assert( is_enum_v<const Enum2>);
            static_assert( is_enum_v<const EnumClass1>);

            static_assert(!is_enum_v<int>);
            static_assert(!is_enum_v<void>);
            static_assert(!is_enum_v<char*>);
            static_assert(!is_enum_v<nullptr_t>);
            static_assert(!is_enum_v<long long>);
